...
const PurchaseItemsDetailPage = () => {
  ...
  const products = userDoc.purchasedItems?.find(
    (item) => item.summaryPurchaseInfo.purchasedId === purchaseId
  );
  ...
  return (
    <>
      {products.purchasedItems.map((product, index) => {
        return (
          <PurchasedItemsDetailsEach
            key={`purchasedItemsEachDetails${index}`}
            product={product}
          />
        );
      })}
