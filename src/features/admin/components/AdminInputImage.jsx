...
const AdminInputImage = ({
  items: { category, productId, dataUploaded },
  getImagesUrl,
  getFromImages,
}) => {
    ...
    () => async (image) => {
      return new Promise((resolve, reject) => {
        ...
        const storageRef = ref(
          storage,
          `${category.toLocaleLowerCase().trim()}/${productId}/` + fileName
        );
  ...
  function getPathStorageFromUrl(url) {
    const baseUrl =
      "https://firebasestorage.googleapis.com/v0/b/shop2-8814c.appspot.com/o/";
  ...
  const removeOneSelectedPhoto = (e, image) => {
    ...
    deleteObject(storageRef)
      .then(() => {
        ...
        const newFromImages = formImages.filter(
          (file) => file.name !== image.name
        );
        setFormImages(newFromImages);
        const newImagesUrl = imagesUrl.filter(
          (url) =>
            getPathStorageFromUrl(url) !==
            `${category}/${productId}/` + image.name
        );
  ...
  return (
    <div className="d-flex flex-column">
        ...
        {formImages?.map((image, index) => {
          return (
            <div
              ...
              style={{ width: "100px" }}
              key={`adminImagesUpdated${index}`}
            >
              {progressBarContent[image.name] === 100 && (
                <span
                  style={{ cursor: "pointer" }}
                  className="m-auto badge text-danger "
                  onClick={(e) => removeOneSelectedPhoto(e, image)}
                >
                  X
                </span>
              )}
              ...
              />
              <div
                className={`progress mt-2 ${!progressBarContent[image.name] ? "d-none" : "d-block"
                  } }`}
              >
                <div
                  key={`progressBar${index}`}
                  className={`progress-bar ${progressBarContent[image.name] === 100 && "bg-success"
                    }`}
                  style={{ width: `${progressBarContent[image.name]}%` }}
                >
                  {Math.floor(progressBarContent[image.name])}%
                </div>
              </div>
              <input
                className="mt-1"
      ...
      {imagesUrl.length !== 0 &&
        formImages.every((image) => progressBarContent[image.name] === 100) && (
          <button className="btn btn-danger mt-4" onClick={removeAllImage}>
            remove all images
          </button>
        )}
    </div>
