...
const EdittingItem = ({ item, getEditedProductId, addedCategory }) => {
  ...
  const [editedItem, setEditedItem] = useState(
    Object.assign(
      {},
      {
        addedImagesUrl: [],
        editedName: name,
        editedCountInStock: countInStock,
        editedPurchasedCount: purchasedCount,
        editedCategory: category,
        editedPrice: price,
        editedImagesUrl: [...imagesUrl],
      }
    )
  );
  const {
  ...
  const abortEditedData = () => {
    if (addedImagesUrl.length) {
      if (
        window.confirm(
          "are you sure to cancle changes ?(all added and deleted images will be lost)"
        ) === true
      ) {
  ...
  const postEditedData = async () => {
    ...
    if (category !== editedCategory) {
      try {
        const isEditedCatExist = categoriesTitle.find(
        ...
        try {
          ...
          objToUpload = {
            imagesUrl: [...dlUrls],
            firstPicture: firstPictureInUi,
            name: editedName,
            category: editedCategory.trim().toLocaleLowerCase(),
            subCategory: editedCategory.trim().toLocaleLowerCase(),
            countInStock: +editedCountInStock,
            price: +editedPrice,
            purchasedCount: +editedPurchasedCount,
            splittedName: editedName.toLocaleLowerCase().split(" "),
            id,
            dateAdded,
            dateEdited: new Date(),
          };
        ...
        if (!isEditedCatExist) {
          ...
          await updateDoc(docRef, {
            categories: [...categoriesTitle, editedCategory],
          });
      ...
      await updateDoc(oldDocRef, {
        imagesUrl: [...editedImagesUrl, ...addedImagesUrl],
        name: editedName,
        countInStock: +editedCountInStock,
        price: +editedPrice,
        purchasedCount: +editedPurchasedCount,
    ...
    <div className="row mb-3" style={{ position: "relative" }}>
      <div className="row mt-3 d-flex border border-1 rounded-3 pt-3 pb-2">
        {!imagesUrl.length ? (
          ...
          !enableEditting && (
            <div
              className="col-xl-3 col-12 d-flex flex-column"
            ...
            >
              <img
                ...
                width={300}
                height={300}
                src={imagesUrl[activeImageNum]}
              />
              {firstPictureInUi === activeImageNum && (
                <button
                  className="btn text-success"
                  style={{ position: "absolute", left: "270px", top: "30px" }}
                >
              ...
              <div className="col-12 d-flex justify-content-between">
                <button
                  ...
                  disabled={activeImageNum === 0 ? true : false}
                ...
                {enableEditting && (
                  <button
                    ...
                    onClick={() => deletePic(imagesUrl[activeImageNum])}
                  >
                ...
                <button
                  ...
                  disabled={
                    imagesUrl.length === 1 ||
                    activeImageNum === imagesUrl.length - 1
                      ? true
                      : false
                  }
          ...
          enableEditting && (
            ...
            >
              <img
                ...
                width={300}
                height={300}
                src={editedImagesUrl[activeImageNum]}
              />
              {firstPictureInUi === activeImageNum && (
                <button
                  className="btn text-success"
              ...
              <div className="col-12 d-flex justify-content-between">
                ...
                <button
                  className="btn border-0 text-primary"
                  disabled={
                    editedImagesUrl.length === 1 ||
                    activeImageNum === editedImagesUrl.length - 1
                      ? true
                      : false
                  }
                  onClick={incrementImageNum}
        ...
        <div className="col-xl-3 col-12 ms-3 lh-base">
          ...
          {!enableEditting ? (
            ...
            <div className="d-flex flex-column">
              <span>category</span>
              <input
                disabled={addedImagesUrl.length}
                name="editedCategory"
                onChange={updateeditedItem}
                type="text"
                defaultValue={category}
              />
              {!categoriesTitle.find(
                (category) => category === editedCategory
              ) && (
                <p className="text-danger">{`you will create new category ${editedCategory}`}</p>
              )}
            </div>
          )}
          {!enableEditting ? (
          ...
          {!enableEditting ? (
            <div>
              {" "}
              <span className="text-primary">warehouse count:</span>
              {countInStock}
            </div>
          ) : (
            <div className="d-flex flex-column">
          ...
          {!enableEditting ? (
            <div>
              {" "}
              <span className="text-primary">sold count:</span> {purchasedCount}
            </div>
          ) : (
            <div className="d-flex flex-column">
              <span>purchased count</span>
              <input
                name="editedPurchasedCount"
                onChange={updateeditedItem}
                type="text"
                defaultValue={purchasedCount}
              />
            </div>
          )}
          <button
            disabled={activeImageNum === firstPictureInUi}
            onClick={setAsActiveImageInDb}
        ...
        <div className="col-xl-5 col-12 d-flex flex-column justify-content-between">
            ...
            <div>
              ...
              {category === editedCategory ? (
                <AdminInputImage
                  ...
                  getFromImages={getFromImages}
                />
              ) : (
                <p className="text-danger mt-3 text-center">
                  you can't change category and add picture at the same time
                </p>
              )}
            </div>
          )}
          ...
          {!enableEditting ? (
            <div className="d-flex flex-column col-5 align-self-end mt-5">
              <button className="btn btn-primary" onClick={enableEditingFunc}>
                ENABLE EDIT MODE
              </button>
              <button className="btn btn-danger mt-3" onClick={removeProduct}>
                DELETE
              </button>
            </div>
          ) : (
            <div className="d-flex justify-content-end mb-4">
              <button className="btn btn-danger me-3" onClick={abortEditedData}>
                ABORT
              </button>
              <button className="btn btn-primary" onClick={postEditedData}>
                <span
                  style={{ width: "15px", height: "15px", fontSize: "10px" }}
                  className={`${uploadingSpinner}`}
                ></span>
                {uploadingSpinner ? "POSTING..." : "POST"}
              </button>
            </div>
          )}
          {/* </div> */}
