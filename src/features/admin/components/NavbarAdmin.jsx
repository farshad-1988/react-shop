...
import SigningAndProfile from "../../shared/components/layout/SigningAndProfile";
import {
...
import QueryResponsive from "../../shared/hooks/QueryResponsive";
  ...
  const dayMessage = () => {
    ...
    return (
      <div className="d-flex flex-column">
        <span>{greet}</span>
        <span>{currentUser?.displayName || null}</span>
      </div>
    );
  };
    ...
    <>
      <nav className="navbar navbar-expand-lg bg-body-tertiary">
        ...
        <FontAwesomeIcon icon={faMicrophone} />
          ...
          <div
            className={`d-flex justify-content-between  ${md && "w-100 mt-2"}`}
          >
            {md && <Link to={"/"}>{homeLogo}</Link>}

            <div className="d-flex align-items-center">
              <Link
                to={"/admin"}
                className=" ms-3 d-flex align-items-center text-decoration-none"
              >
                <button className="d-flex btn btn-success me-4">
