...
const AdminEditProduct = () => {
  const { categoriesTitle, singleCategoryToShow, dispatch, loadingPage } =
  ...
  const removeCategoryCompletely = async () => {
    ...
    const newCatList = catList.categories.filter(
      (category) =>
        category.toLocaleLowerCase() !== selectedCategory.toLocaleLowerCase()
    );
  ...
  const getEditedProductId = (productSpec) => {
    ...
    if (typeof productSpec === "string") {
      newProducts = singleCategoryToShow.filter(
        (product) => product.id !== productSpec
      );
    ...
    <div className="d-flex flex-column" style={{ width: "100%" }}>
      <div className="d-flex align-items-center">
        <select
          ...
          className="w-25 m-auto text-center rounded-2 mt-3 mb-2"
        >
          {categorySelectBox?.map((category, index) => {
            // index===1 && setSelectedCategory(category)
            return (
              <option key={`categoryChoose${index}`} value={category}>
                {category}
              </option>
            );
          })}
      ...
      <div>
        <div
          className="nav-pills d-flex align-items-center ms-3"
          role="tablist"
        >
          <span>sort by:</span>
        ...
        {!loadingPage && !singleCategoryToShow.length && (
          <div className="d-flex justify-content-center mt-5">
            <button
              onClick={removeCategoryCompletely}
          ...
          >
            <div className="m-auto" style={{ width: "95%" }}>
              {singleCategoryToShow?.map((item, index) => {
                return (
                  <EdittingItem
                    key={`editedItemsInCategory${item.id}`}
                    item={item}
                    getEditedProductId={getEditedProductId}
