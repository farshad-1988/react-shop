...
const AdminAddProduct = () => {
  ...
  const submitFormToDB = async (e) => {
    ...
    const objToUpload = {
      ...
      price: +price,
      purchasedCount: 0,
      splittedName: name.toLocaleLowerCase().split(" "),
      id: productId,
      dateAdded: new Date(),
      imagesUrl,
      firstPicture,
    ...
    try {
      ...
      !catData.categories.includes(category) &&
        (await updateDoc(additionalDocRef, {
          categories: [...catData.categories, category],
        }));
      ...
      !categoriesTitle.find(
        ...
        setCategoriesDropDown((prev) => [...prev, categoryRef.current.value]);
  ...
  const updateProductInfo = (e) => {
    ...
    if (e.target.name === "category") {
      ...
      setTimeout(() => {
        setMessageOfNewCategory(
          <p className="text-danger mt-2">{`you will create new category as ${e.target.value}`}</p>
        );
      }, 1000);
    }
  };
  ...
  return (
    <div className="row">
      <div className="d-flex flex-column col-12 col-md-6 m-auto align-items-center">
        ...
        <AdminInputImage
          items={{ category: productInfo?.category, productId, dataUploaded }}
      ...
      <form
        className="container m-auto col-12 col-md-5 row"
        onSubmit={submitFormToDB}
      >
        <div className="mt-2">
          <span>name :</span>
          <input
