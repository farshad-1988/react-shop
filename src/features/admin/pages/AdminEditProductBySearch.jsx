...
const AdminEditProductBySearch = () => {
  ...
  ) : searchedItem.every((arr) => arr.length === 0) ? (
    <h1>
      sorry, there isn't any item with category or name{" "}
      <span className="text-danger">{searchedWord}</span>
    </h1>
  ) : (
    <div>
          ...
          return (
            <div className="m-auto" style={{ width: "95%" }}>
              ...
              {category.map((item, index) => (
                <EdittingItem
                  key={`editedItemsInSearch${item.id}`}
                  item={item}
